...
export default function DevisPage() {
  ...
  const { getDevisByUserId } = useDevis()
  ...
  useEffect(() => {
    if (!authLoading) {
      if (isAuthenticated && user) {
        const userDevis = getDevisByUserId(user.id || 'user-1')
        setDevisList(userDevis)
      }
      setIsLoading(false)
    }
  }, [authLoading, isAuthenticated, user, getDevisByUserId])
                      ...
                      <div className="flex-1">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                          ...
                          <div className="flex items-center gap-2 text-sm">
                            ...
                            <span>{new Date(devis.preferredDate).toLocaleDateString('fr-FR')}</span>
                          </div>
                          <div className="flex items-center gap-2 text-sm">
                            <Clock className="w-4 h-4 text-primary" />
                            <span>{devis.preferredTime}</span>
                          </div>
